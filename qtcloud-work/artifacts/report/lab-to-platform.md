...
八处硬缺口逐条落进文档，两处可信性问题一并修。

1. 上下文来源写死：任务上的动作取任务里记的；工作区上的动作取当前目录并把用的是哪个根印在第一行；不向上搜索、不凭仓库特征猜。（`api-references` 全局选项，`user-guide` 任务段）
2. 新增「输出与退出码」一节：写入型动作都支持 `--dry-run`（只说要写什么、不落盘）。
3. `--json` 与 `--out` 分家：`--json` 把结果输出到标准输出，`--out <文件>` 另存一份；每个动作都支持两者（原先只有三个动作有，且语义混着「导出到文件」）。
4. 标准输出只放结果与 JSON，提示、进度、错误走标准错误；失败时标准错误最后一行给一句「下一步敲什么」。新增「契约与版本」一节：`--json` 字段与退出码只加不改；`--version` 与 `CHANGELOG.md` 头一行、`scripts/validate-version.sh` 对齐。
5. 全局选项里去掉含糊的「约定位置」：`--data` 在 `task --new` 与工作区动作上必须显式给出，不给就报错并说明怎么给。
6. 每个子命令的 `--help` 带一两个例子（写进 `api-references` 开头）。
7. `--next` 的可中断性：先打印一句「这一步交给谁」，Ctrl-C 立刻退且这一步不算过（流水留 ✗），交给 AI 的超时 900 秒。
8. 契约与版本见第 4 条。
